...
class Wav2Vec2FeatureEncoder(nn.Module):
    ...
    def __init__(self, config):
        ...
        if config.feat_extract_module == 'cnn1d':
        ...
        elif config.feat_extract_module == 'cnn3d':
            conv_layers = [Wav2Vec2LayerNormConvLayer3d(config, layer_id=i) for i in range(config.num_feat_extract_layers)]
            
        else:
            raise ValueError(
                f"`config.feat_extract_module` is {config.feat_extract_module}, but has to be one of ['cnn1d','cnn3d']"
            )
    ...
    def forward(self, input_values):
        if DEBUG: 
            print('In Wav2Vec2FeatureEncoder')
...
class Wav2Vec2PreTrainedModel(PreTrainedModel):
    ...
    def _get_feat_extract_output_lengths(
        ...
        # print(input_lengths)
        # assert 1==2,'breakpoint'

        if add_adapter:
            for _ in range(self.config.num_adapter_layers):
                input_lengths = _conv_out_length(input_lengths, 1, self.config.adapter_stride)

        # print('adapter output length')
        # print(input_lengths)
    ...
    def _get_feature_vector_attention_mask(
        ...
        # print('output_lengths: %d'%output_lengths)

        batch_size = attention_mask.shape[0]

        # print('feature_vector_length: %d'%feature_vector_length)
...
class Wav2Vec2Model(Wav2Vec2PreTrainedModel):
    ...
    def forward(
        ...
        if DEBUG:
            print('modeling_wav2vec/Wav2Vec2Model/forward/extract_features')
            print(extract_features.shape)
        
        extract_features = extract_features[:,:,:,0,0]      #SI: squeeze last two as its 1,1 spatial dimension
        if DEBUG:
            print('modeling_wav2vec/Wav2Vec2Model/forward/squeezed_extract_features')
            print(extract_features.shape)


        extract_features = extract_features.transpose(1, 2)
        if DEBUG:
            print('modeling_wav2vec/Wav2Vec2Model/forward/extract_features_transpose')
            print(extract_features.shape)
        ...
        if DEBUG:
            print('modeling_wav2vec/Wav2Vec2Model/forward/projected_extract_features')
            print(hidden_states.shape)
